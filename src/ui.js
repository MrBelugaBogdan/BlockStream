export class UI {
    static createMainMenu(onStart) {
        ...
        const renderWorlds = () => {
            ...
            worlds.forEach((w, index) => {
                ...
                const displayMode = (w.mode || 'survival').toUpperCase();

                row.innerHTML = `
                    <div><b>${w.name || 'Unnamed World'}</b> <br><small style="color:#888">${displayMode}</small></div>
    ...
    static showWorldOptions(e, index, worlds, refresh) {
        ...
        document.getElementById('pop-copy').onclick = () => { 
            const copy = {...worlds[index], name: (worlds[index].name || "World") + " (Copy)"};
            worlds.push(copy); 
            localStorage.setItem('blockstream_worlds', JSON.stringify(worlds)); 
            refresh(); 
            popup.remove(); 
    ...
    static createHotbar(inventoryItems, selectedSlot, hp, engine) {
        ...
        inventoryItems.forEach((name, i) => {
            const inv = (engine.playerData && engine.playerData.inventory) ? engine.playerData.inventory : {};
            const count = engine.gameMode === 'creative' ? '∞' : (inv[name] || 0);
            
            const s = document.createElement('div');
            s.style = `width:50px; height:65px; border:2px solid ${i === selectedSlot ? '#4CAF50' : '#555'}; display:flex; flex-direction:column; align-items:center; color:white; font-size:12px; background:rgba(255,255,255,0.05); border-radius:5px;`;
